 ...
 *     fleet), loops per-peer, resolves peer kappa path, calls syncKappaVaults.
 *   cmdSoulSyncProject: branches on kappa-cwd vs project-cwd; iterates
 *     `project_repos` (kappa branch) or resolves slug → owning kappa →
 *     resolveKappaPath (project branch). Both call syncProjectVault.
 *
 ...
 *   - src/commands/plugins/soul-sync/resolve          (resolveKappaPath, …)
 *
...
// ─── Scratch tmpdir (used by cmdSoulSyncProject kappa-branch existsSync) ───
...
const realFindProjectsForKappa = _rSyncHelpers.findProjectsForKappa;
const realSyncKappaVaults = _rSyncHelpers.syncKappaVaults;
...
const realResolveKappaPath = _rResolve.resolveKappaPath;
const realResolveProjectSlug = _rResolve.resolveProjectSlug;
const realFindKappaForProject = _rResolve.findKappaForProject;
...
interface SyncKappaCall {
  from: string; to: string; fromName: string; toName: string;
}
let syncKappaVaultsCalls: SyncKappaCall[] = [];
let syncKappaVaultsReturns: Array<{ from: string; to: string; synced: Record<string, number>; total: number }> = [];

interface SyncProjectCall {
  projectPath: string; kappaPath: string; projectRepo: string; kappaName: string;
}
let syncProjectVaultCalls: SyncProjectCall[] = [];
let syncProjectVaultReturn = { project: "", kappa: "", synced: {}, total: 0 };

let reportProjectResultCalls: Array<{ project: string; kappa: string; total: number }> = [];

// resolve
let resolveKappaPathMap: Record<string, string | null> = {};
let resolveKappaPathCalls: string[] = [];
let resolveProjectSlugReturn: string | null = null;
let findKappaForProjectReturn: string | null = null;
...
// ssh directly (soul-sync-resolve-kappa.test.ts also mocks this).
mock.module(
...
mock.module(
  ...
  () => ({
    ...
    findProjectsForKappa: (...args: any[]) => {
      if (!mockActive) return (realFindProjectsForKappa as any)(...args);
      findProjectsCalls.push(args[0] as string);
      return findProjectsReturn;
    },
    syncKappaVaults: (...args: any[]) => {
      if (!mockActive) return (realSyncKappaVaults as any)(...args);
      const [from, to, fromName, toName] = args as [string, string, string, string];
      syncKappaVaultsCalls.push({ from, to, fromName, toName });
      return syncKappaVaultsReturns.shift() ?? { from: fromName, to: toName, synced: {}, total: 0 };
    },
    syncProjectVault: (...args: any[]) => {
      if (!mockActive) return (realSyncProjectVault as any)(...args);
      const [projectPath, kappaPath, projectRepo, kappaName] = args as [string, string, string, string];
      syncProjectVaultCalls.push({ projectPath, kappaPath, projectRepo, kappaName });
      return { ...syncProjectVaultReturn, project: projectRepo, kappa: kappaName };
    },
    reportProjectResult: (...args: any[]) => {
      if (!mockActive) return (realReportProjectResult as any)(...args);
      const r = args[0] as { project: string; kappa: string; total: number };
      reportProjectResultCalls.push({ project: r.project, kappa: r.kappa, total: r.total });
    },
  }),
);
...
mock.module(
  ...
  () => ({
    resolveKappaPath: async (...args: any[]) => {
      if (!mockActive) return (realResolveKappaPath as any)(...args);
      const name = args[0] as string;
      resolveKappaPathCalls.push(name);
      return name in resolveKappaPathMap ? resolveKappaPathMap[name] : null;
    },
    resolveProjectSlug: (...args: any[]) => {
      if (!mockActive) return (realResolveProjectSlug as any)(...args);
      return resolveProjectSlugReturn;
    },
    findKappaForProject: (...args: any[]) => {
      if (!mockActive) return (realFindKappaForProject as any)(...args);
      return findKappaForProjectReturn;
    },
  }),
);
...
beforeEach(() => {
  ...
  findProjectsReturn = [];
  findProjectsCalls = [];
  syncKappaVaultsCalls = [];
  syncKappaVaultsReturns = [];
  syncProjectVaultCalls = [];
  syncProjectVaultReturn = { project: "", kappa: "", synced: {}, total: 0 };
  reportProjectResultCalls = [];
  resolveKappaPathMap = {};
  resolveKappaPathCalls = [];
  resolveProjectSlugReturn = null;
  findKappaForProjectReturn = null;
});
...
describe("cmdSoulSync — cwd resolution", () => {
  test("opts.cwd given → tmux display-message NOT called", async () => {
    ...
    await cmdSoulSync(undefined, { cwd: "/home/me/Code/github.com/Org/foo-kappa" });
  ...
  test("opts.cwd empty → tmux display-message called + used as cwd", async () => {
    hostExecResponses = [
      { match: /tmux display-message/, result: "/from/tmux/bar-kappa" },
    ];
    findPeersReturn = [];
    await cmdSoulSync(undefined, {});
    expect(hostExecCalls.some(c => c.includes("tmux display-message"))).toBe(true);
    // kappaName derived from "bar-kappa" (tmux path, -kappa stripped) → findPeers("bar")
  ...
  test("opts.cwd empty + tmux throws → process.cwd() fallback", async () => {
    ...
    process.cwd = () => "/fallback/path/baz-kappa";
    await cmdSoulSync(undefined, {});
    // kappaName from the fallback cwd
    expect(findPeersCalls).toEqual(["baz"]);
  });
});

// ─── cmdSoulSync — kappaName derivation ────────────────────────────────────

describe("cmdSoulSync — kappaName derivation from repo basename", () => {
  test("-kappa suffix stripped from repo basename", async () => {
    findPeersReturn = [];
    await cmdSoulSync(undefined, { cwd: "/a/b/neo-kappa" });
    expect(findPeersCalls).toEqual(["neo"]);
  });

  test(".wt-<name> worktree suffix stripped (regex order: -kappa$ first, then .wt-...)", async () => {
    findPeersReturn = [];
    await cmdSoulSync(undefined, { cwd: "/a/b/neo.wt-1-task" });
    // repoName = "neo.wt-1-task" → no -kappa suffix → strip .wt-... → "neo"
    expect(findPeersCalls).toEqual(["neo"]);
  });
});
...
describe("cmdSoulSync — git rev-parse --git-common-dir handling", () => {
  test("common-dir absolute → kappaPath = join(commonDir, '..')", async () => {
    hostExecResponses = [
      { match: /git -C.*rev-parse --git-common-dir/, result: "/ghq/Org/foo-kappa/.git" },
    ];
    findPeersReturn = ["peer-a"];
    resolveKappaPathMap = { "peer-a": "/ghq/Org/peer-a-kappa" };
    syncKappaVaultsReturns = [{ from: "foo", to: "peer-a", synced: {}, total: 0 }];

    await cmdSoulSync(undefined, { cwd: "/ghq/Org/foo-kappa.wt-1-task" });

    // push direction → from = kappaPath = /ghq/Org/foo-kappa (parent of .git)
    expect(syncKappaVaultsCalls[0].from).toBe("/ghq/Org/foo-kappa");
  });

  test("common-dir relative → joined against cwd", async () => {
    hostExecResponses = [
      { match: /git -C.*rev-parse --git-common-dir/, result: "../../foo-kappa/.git" },
    ];
    findPeersReturn = ["peer-a"];
    resolveKappaPathMap = { "peer-a": "/ghq/Org/peer-a-kappa" };
    syncKappaVaultsReturns = [{ from: "x", to: "y", synced: {}, total: 0 }];

    await cmdSoulSync(undefined, { cwd: "/ghq/Org/foo-kappa.wt-1-task" });

    // join(cwd, commonDir) + ".." → /ghq/Org/foo-kappa
    expect(syncKappaVaultsCalls[0].from).toContain("foo-kappa");
  });

  test("common-dir returns literal '.git' → kappaPath stays as cwd", async () => {
    hostExecResponses = [
    ...
    await cmdSoulSync(undefined, { cwd: "/ghq/Org/foo-kappa" });

    expect(syncKappaVaultsCalls[0].from).toBe("/ghq/Org/foo-kappa");
  });

  test("git rev-parse throws → kappaPath stays as cwd (swallowed)", async () => {
    hostExecResponses = [
    ...
    await cmdSoulSync(undefined, { cwd: "/plain/dir/foo-kappa" });

    expect(syncKappaVaultsCalls[0].from).toBe("/plain/dir/foo-kappa");
  });
});
...
describe("cmdSoulSync — peer resolution", () => {
  test("target given → single-peer list, findPeers NOT consulted", async () => {
    resolveKappaPathMap = { explicit: "/ghq/Org/explicit-kappa" };
    syncKappaVaultsReturns = [{ from: "foo", to: "explicit", synced: {}, total: 0 }];
    const results = await cmdSoulSync("explicit", { cwd: "/a/foo-kappa" });

    expect(findPeersCalls).toEqual([]);
    expect(resolveKappaPathCalls).toEqual(["explicit"]);
  ...
  test("no target + findPeers returns [] → warning + empty results", async () => {
    ...
    const results = await cmdSoulSync(undefined, { cwd: "/a/foo-kappa" });

    expect(results).toEqual([]);
    expect(warns.some(w => w.includes("no sync_peers configured"))).toBe(true);
    expect(warns.some(w => w.includes("'foo'"))).toBe(true);
    // Never reached resolveKappaPath / syncKappaVaults
    expect(resolveKappaPathCalls).toEqual([]);
    expect(syncKappaVaultsCalls).toEqual([]);
  });
  ...
  test("no target + findPeers returns list → loops all peers", async () => {
    ...
    expect(resolveKappaPathCalls).toEqual(["p1", "p2", "p3"]);
    expect(syncKappaVaultsCalls).toHaveLength(3);
...
describe("cmdSoulSync — push vs pull direction", () => {
  test("no --from (default push) → syncKappaVaults(kappaPath, peerPath, kappa, peer)", async () => {
    resolveKappaPathMap = { peer1: "/ghq/Org/peer1-kappa" };
    syncKappaVaultsReturns = [{ from: "foo", to: "peer1", synced: {}, total: 0 }];

    await cmdSoulSync("peer1", { cwd: "/ghq/Org/foo-kappa" });

    const c = syncKappaVaultsCalls[0];
    expect(c.from).toBe("/ghq/Org/foo-kappa");
    expect(c.to).toBe("/ghq/Org/peer1-kappa");
    expect(c.fromName).toBe("foo");
    expect(c.toName).toBe("peer1");
  });

  test("--from (pull) → swaps: syncKappaVaults(peerPath, kappaPath, peer, kappa)", async () => {
    resolveKappaPathMap = { peer1: "/ghq/Org/peer1-kappa" };
    syncKappaVaultsReturns = [{ from: "peer1", to: "foo", synced: {}, total: 0 }];

    await cmdSoulSync("peer1", { cwd: "/ghq/Org/foo-kappa", from: true });

    const c = syncKappaVaultsCalls[0];
    expect(c.from).toBe("/ghq/Org/peer1-kappa");
    expect(c.to).toBe("/ghq/Org/foo-kappa");
    expect(c.fromName).toBe("peer1");
    expect(c.toName).toBe("foo");
  });

  test("pull label logged with arrow from peer → kappa", async () => {
    resolveKappaPathMap = { peer1: "/ghq/Org/peer1-kappa" };
    syncKappaVaultsReturns = [{ from: "peer1", to: "foo", synced: {}, total: 0 }];
    const logs: string[] = [];
...
describe("cmdSoulSync — per-peer handling", () => {
  test("resolveKappaPath returns null → peer skipped, warning logged", async () => {
    findPeersReturn = ["missing"];
    resolveKappaPathMap = { missing: null };
    const warns: string[] = [];
    console.log = (...a: unknown[]) => { warns.push(a.map(String).join(" ")); };
  ...
  test("total === 0 → 'nothing new' per-peer line", async () => {
    findPeersReturn = ["p1"];
    resolveKappaPathMap = { p1: "/ghq/Org/p1-kappa" };
    syncKappaVaultsReturns = [{ from: "foo", to: "p1", synced: {}, total: 0 }];
    const logs: string[] = [];
    console.log = (...a: unknown[]) => { logs.push(a.map(String).join(" ")); };

    await cmdSoulSync(undefined, { cwd: "/a/foo-kappa" });
  ...
  test("mix of skipped + synced peers → results array only includes non-skipped", async () => {
    ...
    resolveKappaPathMap = {
      good: "/ghq/Org/good-kappa",
      bad: null,
      "also-good": "/ghq/Org/also-good-kappa",
    };
    syncKappaVaultsReturns = [
...
// ─── cmdSoulSyncProject — kappa-cwd branch ─────────────────────────────────

describe("cmdSoulSyncProject — kappa-cwd branch", () => {
  test("kappa cwd + no project_repos → warning, empty results", async () => {
    findProjectsReturn = [];
    const warns: string[] = [];
    console.log = (...a: unknown[]) => { warns.push(a.map(String).join(" ")); };

    const results = await cmdSoulSyncProject({ cwd: "/a/b/foo-kappa" });
  ...
  test("kappa cwd + project missing on disk → skip with warning", async () => {
  ...
  test("kappa cwd + project exists → syncProjectVault called (project→kappa)", async () => {
    ...
    syncProjectVaultReturn = {
      project: "theorg/present", kappa: "foo",
...
describe("cmdSoulSyncProject — project-cwd branch", () => {
  test("project cwd (not -kappa) + slug unresolvable → warning, empty results", async () => {
    resolveProjectSlugReturn = null;
    const warns: string[] = [];
    console.log = (...a: unknown[]) => { warns.push(a.map(String).join(" ")); };
  ...
  test("project cwd + slug resolves but no kappa owns it → warning", async () => {
    ...
    expect(results).toEqual([]);
    expect(warns.some(w => w.includes("no kappa owns project 'theorg/lonely'"))).toBe(true);
  });

  test("project cwd + kappa found but kappa repo not found locally → warning", async () => {
    resolveProjectSlugReturn = "theorg/proj";
    findKappaForProjectReturn = "missingkappa";
    resolveKappaPathMap = { missingkappa: null };
    const warns: string[] = [];
    console.log = (...a: unknown[]) => { warns.push(a.map(String).join(" ")); };
    ...
    expect(results).toEqual([]);
    expect(warns.some(w => w.includes("kappa 'missingkappa' repo not found"))).toBe(true);
    expect(syncProjectVaultCalls).toEqual([]);
  });

  test("project cwd + full happy path → syncProjectVault called with project→kappa args", async () => {
    resolveProjectSlugReturn = "theorg/proj";
    findKappaForProjectReturn = "ownerkappa";
    resolveKappaPathMap = { ownerkappa: "/ghq/Org/ownerkappa-kappa" };
    syncProjectVaultReturn = {
      project: "theorg/proj", kappa: "ownerkappa",
    ...
    expect(c.kappaPath).toBe("/ghq/Org/ownerkappa-kappa");
    expect(c.projectRepo).toBe("theorg/proj");
    expect(c.kappaName).toBe("ownerkappa");
    expect(reportProjectResultCalls).toHaveLength(1);
    expect(results).toHaveLength(1);
  });
...
describe("cmdSoulSyncProject — cwd resolution", () => {
  test("opts.cwd empty → tmux display-message called", async () => {
    hostExecResponses = [
      { match: /tmux display-message/, result: "/tmux/path/foo-kappa" },
    ];
    findProjectsReturn = [];
  ...
  test("opts.cwd empty + tmux throws → process.cwd() fallback, project branch triggered", async () => {
    ...
    process.cwd = () => "/fallback/plain-repo"; // not ending in -kappa → project branch
    resolveProjectSlugReturn = null;
    const warns: string[] = [];
    console.log = (...a: unknown[]) => { warns.push(a.map(String).join(" ")); };
