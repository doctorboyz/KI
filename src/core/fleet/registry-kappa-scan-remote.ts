/**
 * registry-kappa-scan-remote — GitHub API kappa discovery.
 *
 * Uses the gh CLI (for auth + pagination) to list repos in target orgs,
 * filters to -kappa suffix, then spot-checks each for a ψ/ directory.
 *
 * The ψ/ check is parallelized in batches (alpha.73, 2026-04-16) — the
 * sequential `execSync gh api` per kappa × ~1.5s × 34 kappas dragged
...
import { deriveName } from "./registry-kappa-scan-local";
...
export async function scanRemote(orgs?: string[], verbose = true): Promise<KappaEntry[]> {
  const config = loadConfig();
  const defaultOrgs = config.githubOrgs || ["doctorboyz", "laris-co"];
  ...
  for (const org of targetOrgs) {
    ...
    try {
      ...
      const kappaRepos = repos.filter(l => l.split(" ")[1]?.endsWith("-kappa"));
      const orgElapsed = ((Date.now() - orgStart) / 1000).toFixed(1);
      console.log(` \x1b[32m✓\x1b[0m ${repos.length} repos, ${kappaRepos.length} kappas (${orgElapsed}s)`);

      // Parallelize ψ/ checks in batches. Sequential = ~1.5s × N kappas.
      ...
      for (let i = 0; i < kappaRepos.length; i += PSI_CHECK_BATCH_SIZE) {
        const batch = kappaRepos.slice(i, i + PSI_CHECK_BATCH_SIZE);
      ...
      console.log();
      console.warn(`  \x1b[33m⚠\x1b[0m [kappa-registry] ${org} failed: ${(err as Error).message?.slice(0, 80)}`);
    }
  }
