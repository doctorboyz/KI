/**
 * registry-kappa-orchestrate — scanAndCache and scanFull entry points.
 *
 * Composes local + remote scan results and writes the merged cache.
 */

import { loadConfig } from "../config";
import type { KappaEntry, RegistryCache } from "./registry-kappa-types";
import { writeCache } from "./registry-kappa-cache";
import { scanLocal } from "./registry-kappa-scan-local";
import { scanRemote } from "./registry-kappa-scan-remote";
...
export async function scanFull(orgs?: string[], verbose = true): Promise<RegistryCache> {
  ...
  if (verbose) console.log(`  \x1b[90m  ${localEntries.length} local kappas found\x1b[0m`);
  const remoteEntries = await scanRemote(orgs, verbose);

  // Merge: local takes priority, remote fills gaps
  const merged = new Map<string, KappaEntry>();
