...
export async function routeComm(cmd: string, args: string[]): Promise<boolean> {
  ...
  if (cmd === "hey" || cmd === "send" || cmd === "tell") {
    ...
    // A user who typed `aoi hey aoijs` (just the target, no message) was
    // previously indistinguishable from `aoi hey` alone — both hit the
    // same "usage:" error. Now the missing-message case names the target
    // so the user sees their input got through.
    if (!target) {
      console.error("usage: aoi hey <target> <message> [--force]");
      ...
      console.error("  e.g. aoi hey aoijs \"hello from neo\"");
      console.error("       aoi hey phaith:01-hojo:3 \"hello hojo-hermes\"");
      throw new UserError("missing target and message");
    }
    if (!msgArgs.length) {
      console.error(`✗ missing message for target '${target}'`);
      console.error(`  aoi hey ${target} <message>`);
      console.error(`  (if '${target}' isn't a valid target, run 'aoi ls' to see available ones)`);
