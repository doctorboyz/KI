/**
 * ki kappa prune — dry-run by default; retires stale/orphan entries from
 * the kappa registry (kappas.json).
 *
...
export interface RetiredEntry extends KappaEntry {
  retired_at: string;
...
function emptyLineage(e: KappaEntry): boolean {
...
export async function runPrune(
  ...
  const entries: KappaEntry[] = (rawCache.kappas as KappaEntry[] | undefined) ?? [];
    ...
    const listAwake = deps.listAwake ?? (async () => {
      ...
      for (const s of sessions) {
        for (const w of s.windows) {
          if (w.name.endsWith("-kappa")) awake.add(w.name.replace(/-kappa$/, ""));
        }
      }
      return awake;
    });
...
export async function cmdKappaPrune(
  opts: PruneOpts = {},
  deps: {
    readEntries?: () => KappaEntry[];
    listAwake?: () => Promise<Set<string>>;
  ...
  const confirmed = await promptConfirm(
    `\n  Retire ${candidates.length} kappa(s)? This moves them to retired[] in the registry (reversible).`,
  );
  ...
  rawCache.kappas = kappas.filter((e) => !retiredNames.has(e.name));
  rawCache.retired = [...existingRetired, ...toRetire];

  writeRawCache(rawCache);

  console.log(`\n  \x1b[32m✓\x1b[0m Retired ${toRetire.length} kappa(s) → retired[] in registry.\n`);
