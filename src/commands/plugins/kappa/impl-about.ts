...
import { resolveKappaSafe } from "./impl-helpers";
import { UserError } from "../../../core/util/user-error";

export async function cmdKappaAbout(kappa: string) {
  const name = kappa.toLowerCase();
  const sessions = await listSessions();

  // Gather all signals first so we can distinguish "real kappa, sparse data"
  // from "no such kappa" — the latter must error rather than print an
  // empty-but-valid-looking record (#390.2).
  const { repoPath, repoName, parentDir } = await resolveKappaSafe(name);
  ...
  try {
    for (const file of readdirSync(fleetDir).filter(f => f.endsWith(".json"))) {
      ...
      const hasKappa = (config.windows || []).some(
        (w: any) => w.name.toLowerCase() === `${name}-kappa` || w.name.toLowerCase() === name
      );
      if (hasKappa) {
        fleetFile = file;
  ...
  // Heading — preserve user input casing. Auto-capitalizing turned 'kijs'
  // into 'Kijs', which is wrong (kappa names are lowercase by convention).
  console.log(`\n  \x1b[36mKappa — ${kappa}\x1b[0m\n`);
  ...
  if (fleetFile) {
    ...
    if (actualWindows > fleetWindowCount) {
      ...
      console.log(`  \x1b[90m  ki fleet init          # regenerate all configs\x1b[0m`);
      console.log(`  \x1b[90m  ki fleet validate      # check for problems\x1b[0m`);
    }
  } else {
