...
/**
 * aoi soul-sync [peer] [--from <peer>] — push ψ/ to peers; --from reverses direction.
 * aoi ss <peer>       push to specific peer
 * aoi ss --from <p>   pull from specific peer
 */
export async function cmdSoulSync(target?: string, opts?: { from?: boolean; cwd?: string }): Promise<SoulSyncResult[]> {
  ...
  if (peers.length === 0) {
    ...
    console.log(`  \x1b[90mAdd "sync_peers": ["name"] to fleet config, or run: aoi ss <peer>\x1b[0m`);
    return results;
  }
...
/**
 * aoi soul-sync --project — absorb project ψ/ into oracle ψ/ (inward only).
