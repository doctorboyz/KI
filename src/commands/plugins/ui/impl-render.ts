...
export function renderUiOutput(opts: UiOptions): string {
  // Detect Shape A: if dist is installed, use aoi-js port (3456) instead of vite (5173)
  ...
  if (opts.tunnel) {
    ...
    return [
      `# Run this on your local machine to forward both lens (${lensPort}) and aoi-js (${AOI_PORT}):`,
      sshCmd,
      ``,
      ...
      ...(distInstalled ? [``, `# (Shape A — aoi-ui dist served from aoi-js on port ${AOI_PORT})`] : []),
    ].join("\n");
  }
