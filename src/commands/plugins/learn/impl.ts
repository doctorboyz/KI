/**
 * ki learn — stub implementation (#521).
 *
 * Migration scaffold for Kappa skill /learn. Real flow (parallel Haiku
 ...
 *   - Points users at the Kappa skill for actual learning runs
 *
...
/**
 * Return a stub message for `ki learn`. Will be replaced by the real
...
export async function cmdLearn(repo: string, mode: LearnMode = "default"): Promise<string> {
  ...
  const lines = [
    `learn: ${mode} mode on "${repo}" — not yet implemented in core plugin; use Kappa skill /learn for full behavior.`,
    `  planned: ${agents} parallel agent(s), write docs to ψ/learn/<owner>/<repo>/YYYY-MM-DD/HHMM_*.md`,
    `  track:   https://github.com/doctorboyz/ki-js/issues/521`,
  ];
