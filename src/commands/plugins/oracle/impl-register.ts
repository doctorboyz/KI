/**
 * aoi oracle register <name> — adopt a discovered oracle into the registry.
 *
...
export async function cmdOracleRegister(
  ...
  if (!name) throw new Error("register requires a name: aoi oracle register <name>");
  ...
  if (!discovered) {
    throw new Error(
      `oracle '${name}' not found in fleet, tmux, or filesystem — try: aoi oracle scan`,
    );
  }
