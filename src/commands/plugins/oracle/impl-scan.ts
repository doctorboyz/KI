...
export async function cmdOracleScan(opts: { force?: boolean; json?: boolean; local?: boolean; remote?: boolean; all?: boolean; verbose?: boolean; quiet?: boolean } = {}) {
  ...
  // AOI_QUIET=1 opts out to the terse summary; --verbose forces loud even
  // under AOI_QUIET.
  ...
  console.log(`  Cache written to \x1b[90m~/.config/aoi/oracles.json\x1b[0m\n`);
}

/**
 * @deprecated Use `cmdOracleList` — `aoi oracle fleet` is now an alias for `aoi oracle ls`.
...
export async function cmdOracleFleet(opts: OracleListOpts = {}) {
  console.error(
    `\x1b[33m⚠  aoi oracle fleet is deprecated — use \x1b[36maoi oracle ls\x1b[0m\x1b[33m instead\x1b[0m`,
  );
