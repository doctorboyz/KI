/**
 * ki kappa scan --stale — read-only staleness classifier (issue #392).
 *
 ...
 *
 * Source of truth: registry cache (kappa entries with local_path) + tmux
 * liveness. For each cloned repo we read the last commit date via git log;
 * missing clones or commit-less repos fall into DEAD.
 */
import { readCache, scanAndCache, listSessions, type KappaEntry } from "../../../../sdk";
...
export async function cmdKappaScanStale(opts: StaleScanOpts = {}): Promise<void> {
  const results = await runStaleScan(opts);

  if (opts.json) {
    console.log(JSON.stringify({ schema: 1, count: results.length, kappas: results }, null, 2));
    return;
  }

  if (results.length === 0) {
    console.log(`\n  \x1b[32m✓\x1b[0m No stale kappas — all clear.\n`);
    return;
  }
  ...
  console.log(`\n  \x1b[36mStale kappa scan\x1b[0m  (${banner})\n`);
