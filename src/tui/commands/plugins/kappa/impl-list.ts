/**
 * ki kappa ls — cached grouped inventory enriched with source-lineage
 * and runtime awake state. Replaces the old awake-only list + fleet view.
 *
 * Reads the kappa registry cache (~/.config/ki/kappas.json) so it shows
 * ALL kappas the user has, not just the ones currently running in tmux.
...
import { lineageOf, timeSince, type KappaLineage } from "./impl-helpers";

export interface KappaListOpts {
  awake?: boolean;
  org?: string;
  json?: boolean;
  scan?: boolean;
...
interface EnrichedEntry {
  ...
  awake: boolean;
  session: string | null;
  lineage: KappaLineage;
}

export async function cmdKappaList(opts: KappaListOpts = {}) {
  ...
  if (shouldRefresh) {
    if (!cache && !opts.json) {
      console.log(
        `\n  \x1b[33m📡\x1b[0m No kappa cache — running first local scan...\n`,
      );
    }
  ...
  //    kappas that haven't landed in the cache yet.
  const sessions = await listSessions().catch(() => []);
  const awakeByName = new Map<string, string>(); // name → session
  for (const s of sessions) {
    for (const w of s.windows) {
      if (w.name.endsWith("-kappa")) {
        const name = w.name.replace(/-kappa$/, "");
        if (!awakeByName.has(name)) awakeByName.set(name, s.name);
      }
    }
  }

  // 3. Merge in tmux-only kappas (just budded, not yet in cache)
  const entries: KappaEntry[] = [...cache!.kappas];
  ...
  console.log(
    `\n  \x1b[36mKappa Fleet\x1b[0m  (${awakeCount} awake / ${total} total)    cache: ${age} ago ${staleMark}\n`,
  );

  if (total === 0) {
    if (opts.awake) console.log("  No awake kappas.\n");
    else if (opts.org) console.log(`  No kappas found in org '${opts.org}'.\n`);
    else
      console.log(
        "  No kappas found. Run \x1b[90mki kappa scan\x1b[0m to refresh.\n",
      );
    return;
  }
...
function formatRow(x: EnrichedEntry, fopts: { showPath: boolean }): string {
  ...
  //   ·  filesystem only (has ψ/ or -kappa suffix but no fleet registration)
  //   faint ·  suspicious (just -kappa suffix, no ψ/, no fleet)
  let icon: string;
  let tag: string;
