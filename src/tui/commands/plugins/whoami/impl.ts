...
/**
 * ki whoami — print the current tmux session name on stdout.
 * Replaces scattered raw `tmux display-message -p '#S'` calls with one
 * canonical, testable command.
 */
export async function cmdWhoami() {
  if (!process.env.TMUX) {
    throw new UserError("ki whoami requires an active tmux session — run 'ki wake <kappa>' or attach to tmux first");
  }
