...
mock.module(join(root, "commands/plugins/sleep/impl"), () => ({
  cmdSleepOne: async (kappa: string, window?: string) => {
    console.log(`sleep ${kappa}${window ? ` window=${window}` : ""}`);
  },
}));
...
describe("sleep plugin", () => {
  it("CLI — valid kappa sleeps ok", async () => {
  ...
  it("CLI — kappa + window sleeps ok", async () => {
  ...
  it("API — valid kappa sleeps ok", async () => {
    const ctx: InvokeContext = { source: "api", args: { kappa: "neo" } };
    const result = await handler(ctx);
    expect(result.ok).toBe(true);
    expect(result.output).toContain("sleep neo");
  });
