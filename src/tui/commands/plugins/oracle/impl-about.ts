...
export async function cmdOracleAbout(oracle: string) {
  ...
  // Heading — preserve user input casing. Auto-capitalizing turned 'aoijs'
  // into 'Aoijs', which is wrong (oracle names are lowercase by convention).
  ...
  if (fleetFile) {
    ...
    if (actualWindows > fleetWindowCount) {
      ...
      console.log(`  \x1b[90m  aoi fleet init          # regenerate all configs\x1b[0m`);
      console.log(`  \x1b[90m  aoi fleet validate      # check for problems\x1b[0m`);
    }
  } else {
