/**
 * aoi oracle ls — cached grouped inventory enriched with source-lineage
 * and runtime awake state. Replaces the old awake-only list + fleet view.
 *
 * Reads the oracle registry cache (~/.config/aoi/oracles.json) so it shows
...
export async function cmdOracleList(opts: OracleListOpts = {}) {
  ...
  if (total === 0) {
    if (opts.awake) console.log("  No awake oracles.\n");
      ...
      console.log(
        "  No oracles found. Run \x1b[90maoi oracle scan\x1b[0m to refresh.\n",
      );
    return;
  }
