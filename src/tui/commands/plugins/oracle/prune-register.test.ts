...
describe("findInFleet", () => {
  it("finds oracle in fleet config by window name", () => {
    ...
    writeFileSync(
      join(dir, "aoijs.json"),
      JSON.stringify({
        windows: [{ name: "aoijs-oracle" }],
        project_repos: ["Soul-Brews-Studio/aoijs-oracle"],
        budded_from: "neo",
        budded_at: "2026-04-07T00:00:00Z",
      }),
    );
    const result = findInFleet("aoijs", dir);
    expect(result).not.toBeNull();
    expect(result!.source).toBe("fleet");
    expect(result!.entry.name).toBe("aoijs");
  ...
  it("returns null if oracle not found in fleet", () => {
    ...
    expect(findInFleet("aoijs", dir)).toBeNull();
  });
...
describe("findInTmux", () => {
  it("finds oracle from tmux window", async () => {
    const sessions = [{ name: "aoijs", windows: [{ name: "aoijs-oracle", index: 0 }] }];
    const result = await findInTmux("aoijs", async () => sessions as any);
    expect(result).not.toBeNull();
